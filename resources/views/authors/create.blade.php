    ...
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 max-w-lg mx-auto">
        <form action="{{ route('authors.store') }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('bio') border-red-500 @enderror" id="bio" name="bio" rows="4">{{ old('bio') }}</textarea>
                @error('bio')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            </div>
